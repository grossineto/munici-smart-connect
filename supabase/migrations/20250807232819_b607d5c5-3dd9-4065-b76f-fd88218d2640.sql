-- Data Retention (90/180/365) and Audit Scope
-- 1) Ensure extensions for scheduling HTTP calls
create extension if not exists pg_net with schema extensions;
create extension if not exists pg_cron with schema extensions;

-- 2) Tenant settings for retention
create table if not exists public.tenant_settings (
  id uuid primary key default gen_random_uuid(),
  tenant_id uuid not null references public.tenants(id) on delete cascade,
  -- Defaults can be 90/180/365 as expected baselines
...
-- Policies: users in tenant can read, only admins can modify
create or replace function public.is_admin()
returns boolean language sql stable security definer set search_path = public as $$
  select coalesce(has_role(auth.uid(), 'admin'::app_role), false)
$$;

create policy if not exists "Tenant users can view tenant_settings"
  on public.tenant_settings for select using (user_has_tenant(tenant_id));

create policy if not exists "Admins can manage tenant_settings"
...
-- 3) Audit logs table and policies
...
-- Allow viewing only for admins or moderators
create policy if not exists "Admins and moderators can view audit logs"
  on public.audit_logs for select
  using (has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'moderator'::app_role));

-- Allow inserts only when executed inside a trigger (prevents manual inserts)
create policy if not exists "Allow inserts via trigger only"
  on public.audit_logs for insert
  with check (pg_trigger_depth() > 0);

-- 4) Generic auditing trigger function
...
-- 5) Attach audit triggers to key tables (skip audit_logs to avoid recursion)
DO $$
BEGIN
  -- Social/News
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'tr_audit_social_mentions') THEN
    CREATE TRIGGER tr_audit_social_mentions AFTER INSERT OR UPDATE OR DELETE ON public.social_mentions
    FOR EACH ROW EXECUTE FUNCTION public.audit_trigger();
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'tr_audit_news_articles') THEN
    CREATE TRIGGER tr_audit_news_articles AFTER INSERT OR UPDATE OR DELETE ON public.news_articles
    FOR EACH ROW EXECUTE FUNCTION public.audit_trigger();
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'tr_audit_news_analysis') THEN
    CREATE TRIGGER tr_audit_news_analysis AFTER INSERT OR UPDATE OR DELETE ON public.news_analysis
    FOR EACH ROW EXECUTE FUNCTION public.audit_trigger();
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'tr_audit_news_alerts') THEN
    CREATE TRIGGER tr_audit_news_alerts AFTER INSERT OR UPDATE OR DELETE ON public.news_alerts
    FOR EACH ROW EXECUTE FUNCTION public.audit_trigger();
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'tr_audit_news_sources') THEN
    CREATE TRIGGER tr_audit_news_sources AFTER INSERT OR UPDATE OR DELETE ON public.news_sources
    FOR EACH ROW EXECUTE FUNCTION public.audit_trigger();
  END IF;

  -- Omnichannel
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'tr_audit_citizens') THEN
    CREATE TRIGGER tr_audit_citizens AFTER INSERT OR UPDATE OR DELETE ON public.citizens
    FOR EACH ROW EXECUTE FUNCTION public.audit_trigger();
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'tr_audit_requests') THEN
    CREATE TRIGGER tr_audit_requests AFTER INSERT OR UPDATE OR DELETE ON public.requests
    FOR EACH ROW EXECUTE FUNCTION public.audit_trigger();
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'tr_audit_messages') THEN
    CREATE TRIGGER tr_audit_messages AFTER INSERT OR UPDATE OR DELETE ON public.messages
    FOR EACH ROW EXECUTE FUNCTION public.audit_trigger();
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'tr_audit_appointments') THEN
    CREATE TRIGGER tr_audit_appointments AFTER INSERT OR UPDATE OR DELETE ON public.appointments
    FOR EACH ROW EXECUTE FUNCTION public.audit_trigger();
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'tr_audit_whatsapp_sessions') THEN
    CREATE TRIGGER tr_audit_whatsapp_sessions AFTER INSERT OR UPDATE OR DELETE ON public.whatsapp_sessions
    FOR EACH ROW EXECUTE FUNCTION public.audit_trigger();
  END IF;

  -- Config/Analytics
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'tr_audit_monitored_keywords') THEN
    CREATE TRIGGER tr_audit_monitored_keywords AFTER INSERT OR UPDATE OR DELETE ON public.monitored_keywords
    FOR EACH ROW EXECUTE FUNCTION public.audit_trigger();
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'tr_audit_analytics_metrics') THEN
    CREATE TRIGGER tr_audit_analytics_metrics AFTER INSERT OR UPDATE OR DELETE ON public.analytics_metrics
    FOR EACH ROW EXECUTE FUNCTION public.audit_trigger();
  END IF;
END $$;

-- Helpful indexes for audit
create index if not exists idx_audit_logs_tenant_time on public.audit_logs(tenant_id, occurred_at desc);
create index if not exists idx_audit_logs_table on public.audit_logs(table_name);

-- 6) Tenants policies (RLS was enabled earlier, add policies to silence linter)
drop policy if exists "Tenants select" on public.tenants;
create policy "Tenants select" on public.tenants for select using (
  user_has_tenant(id) OR is_admin()
);
...
-- 7) Schedule daily retention cleanup via pg_cron calling Edge Function
