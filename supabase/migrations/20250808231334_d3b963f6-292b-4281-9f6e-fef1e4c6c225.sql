...
ALTER TABLE public.perplexity_settings
  ADD CONSTRAINT IF NOT EXISTS perplexity_settings_recency_check CHECK (recency IN ('day','week','month')),
  ADD CONSTRAINT IF NOT EXISTS perplexity_settings_scope_check CHECK (scope IN ('amplo','local','restrito')),
  ADD CONSTRAINT IF NOT EXISTS perplexity_settings_max_articles_check CHECK (max_articles BETWEEN 1 AND 10);
