...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' AND tablename = 'perplexity_settings' 
      AND policyname = 'Users can view their perplexity settings'
  ) THEN
    CREATE POLICY "Users can view their perplexity settings"
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' AND tablename = 'perplexity_settings' 
      AND policyname = 'Users can insert their perplexity settings'
  ) THEN
    CREATE POLICY "Users can insert their perplexity settings"
    ON public.perplexity_settings
    FOR INSERT
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' AND tablename = 'perplexity_settings' 
      AND policyname = 'Users can update their perplexity settings'
  ) THEN
    CREATE POLICY "Users can update their perplexity settings"
    ON public.perplexity_settings
    FOR UPDATE
    USING (auth.uid() = user_id AND public.user_has_tenant(tenant_id));
  END IF;
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' AND tablename = 'perplexity_settings' 
      AND policyname = 'Users can delete their perplexity settings'
  ) THEN
    CREATE POLICY "Users can delete their perplexity settings"
    ON public.perplexity_settings
    FOR DELETE
    USING (auth.uid() = user_id AND public.user_has_tenant(tenant_id));
  END IF;
