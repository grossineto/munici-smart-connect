-- Retry: Multi-tenant migration with compatible policy creation (no IF NOT EXISTS)
...
-- Policies for user_tenants (drop then create)
...
-- 4) Create a default tenant and capture its id, add columns, backfill, constraints, indexes, seed memberships
DO $$
DECLARE
...
BEGIN
  ...
  -- Add tenant_id columns (guarded)
  if not exists (select 1 from information_schema.columns where table_schema='public' and table_name='social_mentions' and column_name='tenant_id') then alter table public.social_mentions add column tenant_id uuid; end if;
  if not exists (select 1 from information_schema.columns where table_schema='public' and table_name='news_articles' and column_name='tenant_id') then alter table public.news_articles add column tenant_id uuid; end if;
  if not exists (select 1 from information_schema.columns where table_schema='public' and table_name='news_analysis' and column_name='tenant_id') then alter table public.news_analysis add column tenant_id uuid; end if;
  if not exists (select 1 from information_schema.columns where table_schema='public' and table_name='news_alerts' and column_name='tenant_id') then alter table public.news_alerts add column tenant_id uuid; end if;
  if not exists (select 1 from information_schema.columns where table_schema='public' and table_name='news_sources' and column_name='tenant_id') then alter table public.news_sources add column tenant_id uuid; end if;

  if not exists (select 1 from information_schema.columns where table_schema='public' and table_name='citizens' and column_name='tenant_id') then alter table public.citizens add column tenant_id uuid; end if;
  if not exists (select 1 from information_schema.columns where table_schema='public' and table_name='requests' and column_name='tenant_id') then alter table public.requests add column tenant_id uuid; end if;
  if not exists (select 1 from information_schema.columns where table_schema='public' and table_name='messages' and column_name='tenant_id') then alter table public.messages add column tenant_id uuid; end if;
  if not exists (select 1 from information_schema.columns where table_schema='public' and table_name='appointments' and column_name='tenant_id') then alter table public.appointments add column tenant_id uuid; end if;
  if not exists (select 1 from information_schema.columns where table_schema='public' and table_name='whatsapp_sessions' and column_name='tenant_id') then alter table public.whatsapp_sessions add column tenant_id uuid; end if;

  if not exists (select 1 from information_schema.columns where table_schema='public' and table_name='monitored_keywords' and column_name='tenant_id') then alter table public.monitored_keywords add column tenant_id uuid; end if;
  if not exists (select 1 from information_schema.columns where table_schema='public' and table_name='analytics_metrics' and column_name='tenant_id') then alter table public.analytics_metrics add column tenant_id uuid; end if;
  ...
  update public.messages m set tenant_id = coalesce(r.tenant_id, default_tenant_id)
    from public.requests r where m.request_id = r.id and m.tenant_id is null;
  ...
  -- FKs (wrapped)
  begin execute immediate 'alter table public.social_mentions add constraint social_mentions_tenant_fk foreign key (tenant_id) references public.tenants(id) on delete restrict'; exception when others then null; end;
  begin execute immediate 'alter table public.news_articles add constraint news_articles_tenant_fk foreign key (tenant_id) references public.tenants(id) on delete restrict'; exception when others then null; end;
  begin execute immediate 'alter table public.news_analysis add constraint news_analysis_tenant_fk foreign key (tenant_id) references public.tenants(id) on delete restrict'; exception when others then null; end;
  begin execute immediate 'alter table public.news_alerts add constraint news_alerts_tenant_fk foreign key (tenant_id) references public.tenants(id) on delete restrict'; exception when others then null; end;
  begin execute immediate 'alter table public.news_sources add constraint news_sources_tenant_fk foreign key (tenant_id) references public.tenants(id) on delete restrict'; exception when others then null; end;
  begin execute immediate 'alter table public.citizens add constraint citizens_tenant_fk foreign key (tenant_id) references public.tenants(id) on delete restrict'; exception when others then null; end;
  begin execute immediate 'alter table public.requests add constraint requests_tenant_fk foreign key (tenant_id) references public.tenants(id) on delete restrict'; exception when others then null; end;
  begin execute immediate 'alter table public.messages add constraint messages_tenant_fk foreign key (tenant_id) references public.tenants(id) on delete restrict'; exception when others then null; end;
  begin execute immediate 'alter table public.appointments add constraint appointments_tenant_fk foreign key (tenant_id) references public.tenants(id) on delete restrict'; exception when others then null; end;
  begin execute immediate 'alter table public.whatsapp_sessions add constraint whatsapp_sessions_tenant_fk foreign key (tenant_id) references public.tenants(id) on delete restrict'; exception when others then null; end;
  begin execute immediate 'alter table public.monitored_keywords add constraint monitored_keywords_tenant_fk foreign key (tenant_id) references public.tenants(id) on delete restrict'; exception when others then null; end;
  begin execute immediate 'alter table public.analytics_metrics add constraint analytics_metrics_tenant_fk foreign key (tenant_id) references public.tenants(id) on delete restrict'; exception when others then null; end;
  ...
  -- Seed memberships from profiles
...
-- 5) RLS: tenant-scoped policies for domain tables (drop then create)
